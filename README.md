# Credits

This game was made possible thanks to incredible free assets found online.  
This list includes all the resources used (or considered) during development — some may not appear in the final game.

---

## Visual Assets

### Fonts & UI
- **Kitchen Sink Font & Sprites** by Polyducks  
  https://polyducks.itch.io/kitchen-sink-textmode-font
- **M6x11 Font** by managore  
  https://managore.itch.io/m6x11
- **M3x6 Font** by managore  
  https://managore.itch.io/m3x6
- **Tiny Cursors** by shmorgus  
  https://shmorgus.itch.io/tiny-cursors
- **Mouse Cursor Pack** by Megabyte Games  
  https://megabyte-games.itch.io/mouse-cursor-pack
- **100 Mouse Cursors Pack (8x8)** by kaboff  
  https://kaboff.itch.io/100-mouse-cursors-pack-8x8
- **UI Sounds** by Kenney  
  https://www.kenney.nl/assets/ui-audio

### Sprites & Tiles
- **Urizen Onebit Tileset** by vurmux  
  https://vurmux.itch.io/urizen-onebit-tileset
- **1-Bit Portraits / Faces** by JayJay99  
  https://jayjay99.itch.io/1-bit-portraits
- **Eight by Eight Things** by dunelec  
  https://dungeon.itch.io/eight-by-eight-things-asset
- **8x8 Enemy Assets** by linker  
  https://linker.itch.io/pixels
- **9x9 Asset Pack (400 sprites)** by kaboff  
  https://kaboff.itch.io/400-sprites-1-bits-pack-9x9
- **16x16 Tree Sprites** by skalding  
  https://skalding.itch.io/bush-sprite-007-8
- **Roguelike 16px Sprites (1-bit)** by ibirothe  
  https://ibirothe.itch.io/roguelike1bit16x16assetpack
- **Tabletop Armies** by cmski  
  https://cmski.itch.io/tabletop-armies-asset-pack
- **Island Assets** by hayaya
  https://hayaya.itch.io/16-x-16-island
- **Island Assets** by piiixl  
  https://piiixl.itch.io/island
  https://piiixl.itch.io/map-tile-set-16px
- **1-bit Animal Pack** by intergenic  
  https://intergenic.itch.io/1-bit-animals-pack
- **Hexany Roguelike Tiles** by hexany-ives  
  https://hexany-ives.itch.io/hexanys-roguelike-tiles
- **Pattern Pack - Chromatic Camouflage** by Megabyte Games  
  https://megabyte-games.itch.io/pattern-pack-chromatic-camouflage
- **Abstract Repeat Patterns** by clastic-artistic  
  https://clastic-artistic.itch.io/caabstractrepeats
- **Sky Photo (Nimbus Clouds)** by Unsplash (2t60-JYQ5pk)  
  https://unsplash.com/photos/nimbus-cumulus-clouds-2t60-JYQ5pk

### Tools
- **Tile Editor** - Tiled
  https://www.mapeditor.org/

---

## Audio

### Background Music
- **Menu Music** — Free Music Pack (Lo-fi / Indie / Metal / Orchestral) by Aila Scott  
  https://ailascott.itch.io/free-music-pack-lo-fi-indie-metal-orchestral
- **Level 1 Music** — *Adventure Begins* from Instrumental Loop Music by fardifferent  
  https://fardifferent.itch.io/loops
- **Fantasy Music Pack for RPG/Platformer** by tommusic  
  https://tommusic.itch.io/fantasy-music-pack-for-rpg-platformer
- **Free Fantasy Music Pack for RPG Adventure / Platformer (Firewood)** by tommusic  
  https://tommusic.itch.io/free-fantasy-music-pack-for-rpg-adventureplatformer-firewood
- **Boss Music** — Free Music Pack by Gutto-Ze  
  https://gutto-ze.itch.io/free-music
- **Additional Instrumental Loops** by fardifferent  
  https://fardifferent.itch.io/loops

### Ambient & Nature Sounds
Sound Effect by
<a href="https://pixabay.com/users/freesound_community-46691455/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=16960">freesound_community</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=16960">Pixabay</a>
- **Calming Rain**  
  https://pixabay.com/sound-effects/calming-rain-257596/
- **Nature Ambience**  
  https://pixabay.com/sound-effects/nature-ambience-323729/
- **Jungle Ambience**  
  https://pixabay.com/sound-effects/jungle-nature-229896/
- **Field Recording - Sweden Springtime Birds**  
  https://pixabay.com/sound-effects/sweden-springtime-birds-field-recording-190420-19629/
- **Bird Sounds**  
  - https://pixabay.com/sound-effects/nature-216798/  
  - https://pixabay.com/sound-effects/nature-birds-singing-217212/  
  - https://pixabay.com/sound-effects/birds-chirping-75156/  
  - https://pixabay.com/sound-effects/birds-19624/  
  - https://pixabay.com/sound-effects/chirping-birds-ambience-217410/  
  - https://pixabay.com/sound-effects/cardinal-37075/  
- **Rainfall**
  - https://pixabay.com/sound-effects/gentle-rainfall-16960/

### Animal Sounds
- Mouse — https://pixabay.com/sound-effects/mouse-6821/  
- Slime — https://pixabay.com/sound-effects/slime-splat-3-219252/  
- Cheetah — https://pixabay.com/sound-effects/lion-roars-with-growls-and-inhales-195839/  
- Horse — https://pixabay.com/sound-effects/horse-neigh-261131/  
- Wolf — https://pixabay.com/sound-effects/wolf-howling-140235/  
- Moose — https://pixabay.com/sound-effects/moose-elk-66771/  
- Elephant (search) — https://pixabay.com/sound-effects/search/elephant/  
- Dino Growl — https://pixabay.com/sound-effects/dinosaur-growl-106744/  
- Monster Sound — https://pixabay.com/sound-effects/monstersound-78617/  

### Sound Effects
- Chick Cluck — https://pixabay.com/sound-effects/short-chick-sound-171389/  
- Bird Flying — https://pixabay.com/sound-effects/pigeons-flying-6351/  
- Wave Start —  
  - https://pixabay.com/sound-effects/tada-military-1-183974/  
  - https://pixabay.com/sound-effects/percussive-drum-hit-1-189787/  
- Laser Bullet — https://pixabay.com/sound-effects/060130-laser-bullet-86975/  
- Hard Disk — https://pixabay.com/sound-effects/skreechy-hard-drive-36584/  
- Crossbow —  
  - https://pixabay.com/sound-effects/crossbows-firing-97671/  
  - https://pixabay.com/sound-effects/bow-release-85040/  
- Running on Gravel — https://pixabay.com/sound-effects/running-on-gravel-301880/  
- Chopping Wood — https://pixabay.com/sound-effects/chopping-wood-with-a-hatchet-50606/  
- Digging a Hole (crop placement) — https://pixabay.com/sound-effects/dig-a-hole-28460/  

#### Pulse Tower Burst (Freesound - Attribution 4.0)
- Balloon burst 1 (NationalTheatret Stasjon, Oslo) by acs272  
  https://freesound.org/s/251404/  
- Short Air Burst by Artninja  
  https://freesound.org/s/739407/  
- Electric Fan by repapaioei9e3  
  https://freesound.org/people/repapaioei9e3/sounds/819061/  
- Fireplace by silencyo  
  https://freesound.org/people/silencyo/sounds/81800/  

### Additional SFX Libraries
- **FPS FX Pack** by lmglolo  
  https://lmglolo.itch.io/free-fps-sfx
- **Misc. SFX** by Floraphonic  
  https://pixabay.com/users/floraphonic-38928062/
- **Creator Chords (music & effects)**  
  https://creatorchords.com/

---

## Resources & References
- **Game Dev StackExchange — Tunneling / Fast-Moving Collisions**  
  https://gamedev.stackexchange.com/questions/192400/in-games-physics-engines-what-is-tunneling-also-known-as-the-bullet-through
